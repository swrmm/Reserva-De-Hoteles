CREATE DATABASE IF NOT EXISTS eva2_hotel;
USE eva2_hotel;
...
CREATE TABLE IF NOT EXISTS reservas (
  ...
  origen VARCHAR(40) NOT NULL DEFAULT 'postman',
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
INSERT INTO habitaciones (numero, tipo, capacidad, precio_noche, estado, descripcion)
VALUES
  ('101', 'individual', 1, 45000, 'disponible', 'Habitacion individual con escritorio y vista interior.'),
  ('202', 'doble', 2, 68000, 'disponible', 'Habitacion doble con cama queen y vista a la ciudad.'),
  ('303', 'suite', 3, 120000, 'mantenimiento', 'Suite con sala privada y tina.')
ON DUPLICATE KEY UPDATE numero = VALUES(numero);

INSERT INTO extras (nombre, precio)
VALUES
  ('Desayuno buffet', 8500),
  ('Estacionamiento', 6000),
  ('Late checkout', 15000)
